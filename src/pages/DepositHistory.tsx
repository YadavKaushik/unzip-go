...
import { ArrowLeft, Copy, Grid2X2, ChevronDown } from 'lucide-react';
...
const paymentTabs = [
  ...
  { key: 'upi-app', label: 'UPI-APP' },
  { key: 'upi-qr', label: 'UPI QR CODE' },
  { key: 'paytm-qr', label: 'PAYTM QR' },
  { key: 'usdt', label: 'USDT' },
];
...
export default function DepositHistory() {
  ...
  useEffect(() => {
    ...
    let query = supabase.from('transactions').select('*')
      .eq('user_id', user.id).eq('type', 'deposit');
    if (activeTab !== 'all') {
      query = query.eq('description', activeTab.toUpperCase());
    }
    ...
    <div style={{ minHeight: '100vh', width: '100%', maxWidth: 430, margin: '0 auto', background: '#f5f5f5', fontFamily: "'Plus Jakarta Sans', Bahnschrift, sans-serif", paddingBottom: 80 }}>
      ...
      <div style={{ background: '#fff', position: 'sticky', top: 0, zIndex: 30, borderBottom: '1px solid #eee' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 14px', height: 48 }}>
          ...
          <span style={{ color: '#333', fontWeight: 800, fontSize: 17 }}>Deposit history</span>
          <div style={{ width: 28 }} />
        </div>
      </div>
      ...
      <div style={{ padding: '12px 10px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            ...
            <p style={{ fontSize: 13, color: '#999' }}>No deposit records yet</p>
          </div>
        ) : (
          <>
            {records.map(txn => {
              ...
              const orderId = `RC${d.getFullYear()}${String(d.getMonth()+1).padStart(2,'0')}${String(d.getDate()).padStart(2,'0')}${txn.id.replace(/-/g,'').slice(0,16)}`;
              ...
              return (
                <div key={txn.id} style={{ background: '#fff', borderRadius: 10, overflow: 'hidden' }}>
                  ...
                  <div style={{
                    ...
                    <span style={{
                      background: '#34be8b', color: '#fff', fontSize: 13, fontWeight: 700,
                  ...
                  <div style={{ padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    <Row label="Type" value={txn.description || 'UPI-QR'} />
