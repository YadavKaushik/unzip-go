...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (flowCount !== 1) {
      return json({ error: "Use either email or phone for registration" }, 400);
    }
    ...
    const profilePatch = isPhoneFlow ? { phone, email: null } : { email };
    await admin.from("profiles").update(profilePatch).eq("user_id", data.user.id);
